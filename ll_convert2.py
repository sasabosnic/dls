...
class BaselineMap:
    ...
    for line in inmap:
	    ...
	    self.map[key] = ( float(lat), float(int) )

    def getLatLong( self, sys, baseline, meridian ):
	lat, int = self.map.get( (sys, baseline, meridian), (None,None) )
	return lat, int
...
def fromDLS( recnum, bmap, lsd, section, township, range, meridian ):
    if lsd < 1 or lsd > 16:
	print("The lsd number on record %d must be in the range [1,16]" % recnum, file=sys.stderr)
	return None, None
    if section < 1 or section > 36:
	print("The section number on record %d must be in the range [1,36]" % recnum, file=sys.stderr)
	return None, None
    if township < 1 or township > 126:
	print("The township number on record %d must be in the range [1,126]" % recnum, file=sys.stderr)
	return None, None
    if range < 1 or range > 35:
	print("The range number on record %d must be in the range [1,35]" % recnum, file=sys.stderr)
	return None, None
    if meridian < 1 or meridian > 6:
	print("The meridian number on record %d must be in the range [1,6]" % recnum, file=sys.stderr)
	return None, None
	...
	print("Invalid survey system for input record %d" % recnum, file=sys.stderr)
	return None, None
	...
	print("Unable to lookup baseline for %s on input record %d" % ( dls, recnum ), file=sys.stderr)
	return None, None
    if baselat == '0':
	print("Found a 0 latitude looking up baseline for %s on input record %d" % ( dls, recnum ), file=sys.stderr)
	return None, None
...
class FirstBlock( BCBlock ):
	...
	return 40+lat+sublat, 48+int+sublong
...
def fromBCNTS( recnum, blocks, b0, b1, b2, b3, b4, b5 ):
	...
	print("The bc nts coordinate MM **-**-**/MM-**-** must be in the set " + ",".join(x for x in list(bc_mapnums.items( )) ) + " on input record %d" % recnum, file=sys.stderr)
	return None, None

    if b1 < 1 or b1 > 16:
	print("The bc nts coordinate MM **-**-**/**-MM-** must be in the range ['A','P']" + " on input record %d" % recnum, file=sys.stderr)
	return None, None
    if b2 < 1 or b2 > 16:
	print("The bc nts coordinate MM **-**-**/**-**-MM must be in the range [1,16]" + " on input record %d" % recnum, file=sys.stderr)
	return None, None
    if b3 < 1 or b3 > 16:
	print("The bc nts coordinate MM **-**-MM/**-**-** must be in the range ['A','L']" + " on input record %d" % recnum, file=sys.stderr)
	return None, None
    if b4 < 1 or b4 > 100:
	print("The bc nts coordinate MM **-MM-**/**-**-** must be in the range [1,100]" + " on input record %d" % recnum, file=sys.stderr)
	return None, None
    if b5 < 1 or b5 > 4:
	print("The bc nts coordinate MM MM-**-**/**-**-** must be in the range ['A','D']" + " on input record %d" % recnum, file=sys.stderr)
	return None, None

    lat, int = blocks.getLatLong( [b0, b1, b2, b3, b4, b5] )
    return lat, -int
...
def convert( separator, replace_latlong, bmap, blocks ):
	...
	if len( fields ) != len( columns ):
	    print("Expected %d fields on record %d of input" % ( len( columns ), recnum ), file=sys.stderr)
	    exit( 1 )
	...
	if replace_latlong or fields[lat_col] == "" or fields[long_col] == "":
		...
		lat, int = fromBCNTS( recnum, blocks, b0, b1, b2, b3, b4, b5 )
	    else:
		print("Poorly formated dls or nts location '%s' on record %d of input" % ( dls, recnum ), file=sys.stderr)
...
"""
	...
	print("Must specify name of the file containing the baseline map data.", file=sys.stderr)
	parser.print_help( )
	sys.exit( 2 )
